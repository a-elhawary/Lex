...
function CasesPageInner() {
  ...
  const [reportType, setReportType] = useState<
    "general" | "litigation" | "complete" | "hearings" | null
  >(null);
  ...
  const handleStatusChange = (status: string) => {
    ...
    if (status === "All" || status === "") {
      ...
      router.push(`/cases?status=${encodeURIComponent(status)}`);
    }
  };
      ...
      case "general":
        return (
          <div className="space-y-4">
            ...
            <div>
              <strong>Attorney:</strong> {selectedCase.attorney}
            </div>
            <div>
              <strong>Description:</strong> {selectedCase.description}
            </div>
            <div>
              <strong>Created:</strong> {selectedCase.dateCreated}
            </div>
            <div>
              <strong>Next Hearing:</strong> {selectedCase.nextHearing || "Not scheduled"}
            </div>
          </div>
        );
      case "litigation":
        return (
          <div className="space-y-4">
            ...
            <div>
              <strong>Case Timeline:</strong> Initial consultation on {selectedCase.dateCreated}, next hearing scheduled for {selectedCase.nextHearing || "TBD"}
            </div>
            <div>
              <strong>Key Events:</strong> Case filed, discovery in progress, mediation scheduled
            </div>
          </div>
        );
      case "complete":
              ...
              {caseTimeEntries.length > 0 ? (
                <ul className="list-disc list-inside ml-4">
                  {caseTimeEntries.map((entry) => (
                    <li key={entry.id}>
                      {entry.date}: {entry.hours}h - {entry.description}
                    </li>
                  ))}
                </ul>
              ) : (
      ...
      case "hearings":
        return (
          <div className="space-y-4">
            ...
            <div>
              <strong>Case Title:</strong> {selectedCase.title}
            </div>
            <div>
              <strong>Attorney:</strong> {selectedCase.attorney}
            </div>
            <div>
              <strong>Next Hearing:</strong> {selectedCase.nextHearing || "No upcoming hearings"}
            </div>
            <div>
              ...
              {selectedCase.pastHearings && selectedCase.pastHearings.length > 0 ? (
                <ul className="list-disc list-inside ml-4">
                  {selectedCase.pastHearings.map((hearing, index) => (
                    <li key={index}>
                      {hearing.date}: {hearing.type} - {hearing.outcome}
                    </li>
                  ))}
                </ul>
              ) : (
            ...
            <div>
              <strong>Case Status:</strong> {selectedCase.status}
            </div>
          </div>
        );
      default:
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Manage and track all your legal cases</p>
        </div>
        <Button className="bg-primary text-primary-foreground hover:bg-primary/90">New Case</Button>
      </div>
                  ...
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Client: {case_.client}</p>
                      </div>
                      <div className="flex gap-2">
                        <Badge className={getCategoryColor(case_.category)}>{case_.category}</Badge>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <Link
                          href={`/teams/${encodeURIComponent(case_.attorney)}`}
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground">{case_.nextHearing || "Not scheduled"}</p>
                        </div>
                      </div>
      ...
      {selectedCase && reportType && (
        <Dialog open={!!selectedCase && !!reportType} onOpenChange={closeReport}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle>
                {reportType.charAt(0).toUpperCase() + reportType.slice(1)} Report: {selectedCase.title}
              </DialogTitle>
            </DialogHeader>
            {renderReportContent()}
